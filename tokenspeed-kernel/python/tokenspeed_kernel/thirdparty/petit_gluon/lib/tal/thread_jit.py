...
from triton.experimental.gluon.language._core import builtin, _unwrap_if_constexpr, distributed_type
from triton.language.core import constexpr_type
from triton.compiler.code_generator import CodeGenerator, ASTFunction
from triton.backends.amd.compiler import HIPBackend
from triton.backends.compiler import GPUTarget
from triton._C.libtriton import ir, llvm, amd, passes
from triton.runtime.cache import get_cache_manager
from tokenspeed_kernel.thirdparty.petit_gluon.lib.tal.device import source_key, _CompileTimeView
from tokenspeed_kernel.thirdparty.petit_gluon.lib.gemm.rocm.amd_intrinsics import _native_call


def _memory_code(dtype):
    return 'f32' if dtype == l.float32 else 'f16' if dtype == l.float16 else 'i' + str(dtype.primitive_bitwidth)


@builtin
def _load(ptr, mask=None, other=None, boundary_check=(), padding_option='', cache_modifier='',
          eviction_policy='', volatile=False, _semantic=None):
    if ptr.type.is_block() or mask is not None or boundary_check or padding_option or cache_modifier or eviction_policy or volatile:
        raise ValueError('thread_jit load requires a scalar pointer; spell conditional accesses with if')
    dtype = ptr.dtype.element_ty
    code = _memory_code(dtype)
    value = _native_call('memory.load.' + code, code, ('p' + str(_unwrap_if_constexpr(ptr.dtype.address_space)),), (ptr,), False, _semantic=_semantic)
    return _semantic.bitcast(value, dtype)


@builtin
def _store(ptr, value, mask=None, boundary_check=(), cache_modifier='', eviction_policy='', _semantic=None):
...
def _atomic_add(ptr, val, mask=None, sem=None, scope=None, _semantic=None):
    if ptr.type.is_block() or mask is not None or sem not in (None, 'relaxed') or scope not in (None, 'gpu'):
        raise ValueError('thread_jit atomic_add supports scalar native relaxed agent atomics')
    dtype = ptr.dtype.element_ty
    code = _memory_code(dtype)
    if code not in ('i32', 'i64'):
        raise ValueError('thread_jit atomic_add currently supports native integer counters')
    value = _semantic.cast(_semantic.to_tensor(val), dtype)
    return _native_call('memory.atomic.add.' + code, code,
...
class _BoundThreadDeviceMethod:
    ...
    def __call__(self, *args, _semantic=None, _generator=None, **kwargs):
        return self.method.jit(l.constexpr(_CompileTimeView(self.obj)), *args,
                              _semantic=_semantic, _generator=_generator, **kwargs)
...
def _scalar_type(value, tensors):
    ...
    if isinstance(value, l.tuple):
        return l.tuple_type([_scalar_type(v, tensors) for v in value.values], value.type.fields)
...
def _lower_device(mod, backend, options):
    ...
    libraries = dict(re.findall(r'libname = "(petit_native_[0-9a-f]+)", libpath = "([^"]+)"', str(mod)))
...
def _disable_atomic_or_loop_unrolling(source):
    ...
    for index, line in enumerate(lines):
        if not any(('call ' in line and '@' + symbol + '(' in line)
                   for symbol in atomic_adapters):
            continue
        block_begin = index
        while block_begin >= 0 and not re.fullmatch(r'[A-Za-z$._][\w$.-]*:',
                                                    lines[block_begin].strip()):
            block_begin -= 1
        block_end = index
        while block_end < len(lines) and not lines[block_end].lstrip().startswith('br '):
            block_end += 1
...
class thread_jit:
    ...
    def _compile(self, arg_types, num_warps, arch):
        key = sha256((self.cache_key + str([t.mangle() for t in arg_types]) + str(num_warps) + arch).encode()).hexdigest()
        if key in self._compiled:
            return self._compiled[key]
        library = 'petit_native_' + key
        name, wrapper = library + '_body', library + '_call'
        backend = HIPBackend(GPUTarget('hip', arch, 64))
        options = backend.parse_options(dict(num_warps=num_warps, enable_fp_fusion=False, sanitize_overflow=False))
        context = ir.context()
        ...
        begin_line = getattr(self.jit, 'def_file_line_number', self.jit.starting_line_number + 1)
        generator = CodeGenerator(context, prototype, self.jit.get_capture_scope(), name,
            self.jit, options=options, codegen_fns=backend.get_codegen_implementation(options), module_map=module_map,
            is_gluon=True, is_kernel=False, file_name=file_name, begin_line=begin_line)
        for attr, value in [('ttg.num-warps', num_warps), ('ttg.num-ctas', 1), ('ttg.threads-per-warp', 64)]:
            generator.module.set_attr(attr, generator.builder.get_int32_attr(value))
        generator.module.set_attr('ttg.target', generator.builder.get_string_attr('hip:' + arch))
        generator.visit(self.jit.parse())
        generator.fn.set_attr('sym_visibility', generator.builder.get_string_attr('public'))
        ...
        lines = source.splitlines()
        start = next(i for i, line in enumerate(lines) if line.startswith('define ') and '@' + name + '(' in line)
        ...
        for param in params[-2:]:
            ...
            if re.search(re.escape(variable) + r'\b', '\n'.join(lines[start + 1:end])):
                raise ValueError('thread_jit scratch allocation must remain in the outer kernel')
        ...
        lines[start] = head + '@' + name + '(' + ', '.join(params) + ') alwaysinline convergent {'
        ...
        if returned == 'void':
            wrapper_ir = 'define i32 @' + wrapper + '(' + ', '.join(params) + ') alwaysinline convergent {\n'
            wrapper_ir += 'call void @' + name + '(' + ', '.join(params) + ')\nret i32 0\n}\n'
        else:
            wrapper_ir = 'define ' + returned + ' @' + wrapper + '(' + ', '.join(params) + ') alwaysinline convergent {\n'
            wrapper_ir += '%value = call ' + returned + ' @' + name + '(' + ', '.join(params) + ')\nret ' + returned + ' %value\n}\n'
        source = '\n'.join(lines) + '\n' + wrapper_ir
    ...
    def __call__(self, *args, _semantic=None, _generator=None, **kwargs):
        if _generator is None:
            raise TypeError('thread_jit functions are called from an outer @gluon.jit kernel')
        bound = self._signature.bind(*args, **kwargs)
        bound.apply_defaults()
        values = []
        arg_types = tuple(_scalar_type(bound.arguments[n], values) for n in self.jit.arg_names)
        library, path, symbol, result_type = self._compile(arg_types, _semantic.builder.options.num_warps, _semantic.builder.options.arch)
        ...
        ty = distributed_type(dtype, values[0].type.shape, values[0].type.layout) if values[0].type.is_block() else dtype
        handle = _semantic.builder.create_extern_elementwise(library, path, symbol,
            [v.handle for v in values], ty.to_ir(_semantic.builder), False)
