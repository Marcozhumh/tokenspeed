...
from tokenspeed_kernel.thirdparty.petit_gluon.lib.gemm.rocm.amd_intrinsics import (
    BufferResource, amdgcn_ds_swizzle, amdgcn_mov_dpp, amdgcn_shuffle,
...
from tokenspeed_kernel.thirdparty.petit_gluon.lib.moe.rocm.quantization import AiterMxFp4Quantization, QuantizeMxFp4
from tokenspeed_kernel.thirdparty.petit_gluon.lib.moe.rocm.ops.stage1_accumulator_lds import StoreStage1AccumulatorLds2D, StoreStage1AccumulatorLdsM64
...
class MxFp4ActivationQuantizer(DeviceTemplate):
    def __init__(self, Config):
        ...
        self.kInputFragments = self.kRowsPerTile * self.kTileCols // (Config.kNumWarps * 64) // 4
    ...
    def StoreAccumulator(self, shm, h, wid, wtid):
        if self.kWaveM64:
            StoreStage1AccumulatorLdsM64(shm, h, wid, wtid, self.kTileCols, self.Config.kNumWarps)
        else:
            StoreStage1AccumulatorLds2D(shm, h, wid, wtid, self.kRowsPerTile,
                self.kTileCols, self.Config.kStage1WarpsM, self.Config.kStage1WarpsN)
    ...
    @device_method
    def Store(self, workspace, value_base, scale_base, value_row, scale_row, tile_n,
              col_lane, inter_dim, scale_cols, quantized, kStoreScope: l.constexpr=BufferResource.kNone, valid=True):
        col_local = col_lane * 4
        value_offset = value_row * (inter_dim // 2) + tile_n * (self.kTileCols // 2) + col_local // 2
        partner = amdgcn_mov_dpp(quantized.value.to(l.uint32), 0xb1, 0xf, 0xf, False)
        packed = quantized.value.to(l.uint32) | (partner << 16)
        BufferResource.StoreU32(workspace, value_offset, value_base, packed,
                               kStoreScope | BufferResource.kNTBit, valid & ((col_lane & 1) == 0))
        ...
        scale_offset = MxFp4ActivationLayout.ScaleOffset(scale_row, scale_col, scale_cols)
        BufferResource.StoreU32(workspace, scale_offset, scale_base, packed,
                               kStoreScope, valid & ((col_lane & 31) == 0))
...
class MxFp4Stage2Input(DeviceTemplate):
    def __init__(self, Config, InputRegs=MxFp4InputRegs):
        ...
        self.kLdsStages, self.kLdsVectorsPerRow, self.kInvalidValueOffset = 2, 16, 0xfffffff0
        self.kInputShmWords = self.kLdsStages * self.kRowsPerTile * self.kLdsVectorsPerRow * 4

    @device_method
    def LoadTile(self, workspace, value_voffset, value_soffset, scale_voffset, scale_soffset,
                 tile_k, valid, wtid, kAux: l.constexpr=BufferResource.kNone):
        value_offset = l.where(valid, value_voffset + tile_k * self.kGroupDim // 2, self.kInvalidValueOffset)
        value = llvm_amdgcn_raw_buffer_load_v4i32(_resource_content(workspace), value_offset, value_soffset, kAux)
        return Stage2Prefetch(value, self.LoadScaleWord(workspace, scale_voffset, scale_soffset, tile_k, wtid, kAux))

    @device_method
    def StoreLds(self, shm, value, stage, tid):
        row = tid // self.kVectorsPerRow
        vector = tid % self.kVectorsPerRow
        _store_vector4(shm + ((stage * self.kRowsPerTile + row) * self.kLdsVectorsPerRow + (vector ^ (row & 15))) * 4, value)
    ...
    def ReadLds(self, shm, stage, scale, wtid):
        ...
        for row_group in l.static_range(2):
            for k128 in l.static_range(2):
                ...
                x += (_load_vector4(shm + ((stage * self.kRowsPerTile + row) * self.kLdsVectorsPerRow + (vector ^ (row & 15))) * 4),)
        return MxFp4InputRegs(x, (scale,))

    @device_method
    def LoadScaleWord(self, workspace, scale_voffset, scale_soffset, tile_k, wtid, kAux: l.constexpr):
        load_row = wtid & 31
        load_col = tile_k * 8 + (wtid >> 5) * 4
        loaded = BufferResource.LoadU32(workspace, scale_voffset + load_row * self.kScaleCols + load_col, scale_soffset, kAux)
        row16 = wtid & 15
        scale4 = wtid >> 4
        shift = scale4 * 8
        return ((amdgcn_shuffle(loaded, row16) >> shift) & 0xff) | (((amdgcn_shuffle(loaded, row16 + 16) >> shift) & 0xff) << 8) | (((amdgcn_shuffle(loaded, row16 + 32) >> shift) & 0xff) << 16) | (((amdgcn_shuffle(loaded, row16 + 48) >> shift) & 0xff) << 24)
