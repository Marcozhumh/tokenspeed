...
from tokenspeed_kernel.thirdparty.petit_gluon.lib.gemm.rocm.amd_intrinsics import BufferResource, amdgcn_wave_inclusive_add, amdgcn_shuffle, amdgcn_ballot, amdgcn_ctz64
    ...
    def FetchRecvSumPerExpert(self, state, wtid):
        ...
        for i in l.static_range(self.kNumExpertsPerLane):
            ...
            if expert < self.kNumExpertsPerRank:
                ...
                while pending:
                    value = BufferResource.LoadU64(state.workspace_.br_, expert * 8, self.Workspace.RecvSumCounterOffset(0), BufferResource.kSC1Bit)
    ...
    def GetWork(self, state, wtid, work_id):
        ...
        for expert in l.static_range(self.kNumExpertsPerRank):
            ...
            if prev <= work_id and work_id < next:
                return True, l.full((), expert, l.uint32), l.minimum(self.kSortedTokenBlock, tokens - (work_id - prev) * self.kSortedTokenBlock)
...
class MegaMoETwoStageScheduler(MegaMoEScheduler):
    ...
    def GetWork(self, state, wtid, logical_id):
        last_metadata = amdgcn_shuffle(state.expert_metadata_[(self.kNumExpertsPerRank - 1) // 64], (self.kNumExpertsPerRank - 1) % 64)
        last_tokens = last_metadata & self.kTokenCountMask
        total_blocks = (last_metadata >> self.kTokenCountBits) + (last_tokens + self.kSortedTokenBlock - 1) // self.kSortedTokenBlock
        ...
        for i in l.static_range(self.kNumExpertsPerLane):
            ...
            lane_blocks = (lane_tokens + self.kSortedTokenBlock - 1) // self.kSortedTokenBlock
            matches = amdgcn_ballot((i * 64 + wtid < self.kNumExpertsPerRank) & (lane_base <= target_block) & (target_block < lane_base + lane_blocks)) & self.kExpertLaneMask
            if matches != 0:
                ...
                work = Work(phase, i * 64 + expert, target_block, target_block * self.kSortedTokenBlock,
                    l.minimum(self.kSortedTokenBlock, tokens - block_in_expert * self.kSortedTokenBlock),
    ...
    def GetStage1Work(self, state, wtid, logical_id):
        ...
        if self.kStage1M == self.kSortedTokenBlock:
            for i in l.static_range(self.kNumExpertsPerLane):
                ...
                lane_base = lane_metadata >> self.kTokenCountBits
                lane_blocks = (lane_tokens + self.kSortedTokenBlock - 1) // self.kSortedTokenBlock
                matches = amdgcn_ballot((i * 64 + wtid < self.kNumExpertsPerRank) & (lane_base <= target_stage1_block) & (target_stage1_block < lane_base + lane_blocks)) & self.kExpertLaneMask
                if matches != 0:
                    expert = amdgcn_ctz64(matches)
                    metadata = amdgcn_shuffle(lane_metadata, expert)
                    tokens = metadata & self.kTokenCountMask
                    ...
                    return True, Work(l.full((), 0, l.uint32), i * 64 + expert, target_stage1_block,
                        target_stage1_block * self.kSortedTokenBlock,
                        l.minimum(self.kStage1M, tokens - block_in_expert * self.kStage1M), tile)
            return False, self.EmptyWork()
        ...
        while expert < self.kNumExpertsPerRank and not found:
            ...
            if self.kNumExpertsPerLane == 2:
                lane_metadata = l.where(expert < 64, state.expert_metadata_[0], state.expert_metadata_[1])
            metadata = amdgcn_shuffle(lane_metadata, expert % 64)
            tokens = metadata & self.kTokenCountMask
            stage1_blocks = (tokens + self.kStage1M - 1) // self.kStage1M
            physical_blocks = (tokens + self.kSortedTokenBlock - 1) // self.kSortedTokenBlock
            if stage1_block_base <= target_stage1_block and target_stage1_block < stage1_block_base + stage1_blocks:
                block_in_expert = target_stage1_block - stage1_block_base
                pool_block = physical_block_base + block_in_expert * (self.kStage1M // self.kSortedTokenBlock)
                found = l.full((), True, l.int1)
                work = Work(l.full((), 0, l.uint32), expert.to(l.uint32), pool_block,
                    pool_block * self.kSortedTokenBlock, l.minimum(self.kStage1M, tokens - block_in_expert * self.kStage1M), tile)
