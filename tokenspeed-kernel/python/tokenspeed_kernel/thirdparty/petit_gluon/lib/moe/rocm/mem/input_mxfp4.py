...
from tokenspeed_kernel.thirdparty.petit_gluon.lib.moe.rocm.memory_ops import MakeBufferResource, _load_vector4, _register_array_get
from tokenspeed_kernel.thirdparty.petit_gluon.lib.gemm.rocm.amd_intrinsics import amdgcn_thread_id
...
class MxFp4Input(DeviceTemplate):
    def __init__(self, Config):
        ...
        self.kDim, self.kTokenBatch, self.kNumWarps = Config.kDim, Config.kTokenBatch, Config.kNumWarps
        ...
        self.kK128Tiles, self.kK256Tiles, self.kK32PerTile = self.kGroupK // 128, self.kGroupK // 256, 4
        ...
        self.kActivationFragments, self.kScaleFragments = 2 * self.kK128Tiles, self.kK256Tiles
        ...
        self.kShmScaleWords = self.kScaleFragments * self.kScaleWordsPerM32 * self.kWarpsM
        self.RowVecLayout = Layout(Shape(self.kK128Tiles, self.kK32PerTile), Stride(self.kK32PerTile, 1))
    ...
    @device_method
    def Initialize(self, value_ptr, scale_ptr, dim, m, n, route_group, route_group_limit):
        return InputState(MakeBufferResource(value_ptr, m * self.kDim // 2),
            MakeBufferResource(scale_ptr, route_group_limit * self.kWarpsM * self.kDim),
            route_group, l.full((), 0, l.uint32), l.full((), 0, l.uint32))
    ...
    def FetchAsync(self, state, shm_x, wid, wtid, tokens):
        ...
        for load in l.static_range(self.kLoadIterations):
            ...
            byte_offset = (_register_array_get(tokens, token_idx) * self.kRowVecs + offset
                           + row_vec_layout(make_coord(k128, k32))) * 16
            # Both supported geometries have complete wave loads.
            l.static_assert(self.kAsyncVecsPerWarp == self.kLoadIterations * 64)
            BufferResource.LoadLds(state.values_, lds_ptr, byte_offset, 0, BufferResource.kNone, 16, 0)
        return InputState(state.values_, state.scales_, state.route_group_, values_offset_vec, state.scales_offset_vec_)
    ...
    def FetchScaleAsync(self, state, shm_scale, wid, wtid, tokens, m):
        ...
        BufferResource.LoadLds(state.scales_, lds_ptr, src_word * 4, 0,
                               BufferResource.kNone, 4, 0, predicate=wid < self.kWarpsM)
    ...
    def FetchToRegs(self, state, shm_x, wtid):
        ...
        k1 = shm_x + (row_base + self.SwizzledVector(row, vector + self.kK32PerTile)) * 4
        kNextRow: l.constexpr = self.kMmaRows * self.kRowVecsPerTile
        return (_load_vector4(k0), _load_vector4(k1),
                _load_vector4(k0 + kNextRow * 4), _load_vector4(k1 + kNextRow * 4))
    ...
    def AdvanceScaleStep(self, state):
        return InputState(state.values_, state.scales_, state.route_group_,
                          state.values_offset_vec_, state.scales_offset_vec_ + self.kScaleFragments)
