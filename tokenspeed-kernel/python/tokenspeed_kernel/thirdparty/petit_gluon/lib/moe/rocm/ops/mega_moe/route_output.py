...
from tokenspeed_kernel.thirdparty.petit_gluon.lib.gemm.rocm.amd_intrinsics import BufferResource, amdgcn_readfirstlane, amdgcn_pk_add_f32, amdgcn_cvt_pk_bf16_f32
...
class MegaMoETwoStage2Epilogue(TwoStageStage2Epilogue):
    def __init__(self, TileSchedule):
        ...
        self.kPayloadLoadAux = BufferResource.kSC1Bit if self.Config.kNumRanks > 1 else BufferResource.kNone

    @device_method
    def LoadRouteWeights(self, context, wtid):
        offset = self.Workspace.L1TokenWeightsOffset(context.workspace.rank_id_, context.pool_base)
        lane = wtid % 16
        packed = (BufferResource.LoadU32(context.workspace.br_, lane * 4, offset, self.kPayloadLoadAux),
                  BufferResource.LoadU32(context.workspace.br_, (lane + 16) * 4, offset, self.kPayloadLoadAux))
        return packed[0].to(l.float32, bitcast=True), packed[1].to(l.float32, bitcast=True)
    ...
    def WriteBack(self, context, shm, tile_col, wid, wtid):
        ...
        for row_group in l.static_range(self.kTileRows // self.kNumWarps):
            ...
            if valid:
                metadata = BufferResource.LoadU64(context.workspace.br_, 0,
                    self.Workspace.TokenMetadataOffset(context.workspace.rank_id_, context.pool_base + row), self.kPayloadLoadAux)
            ...
            output_row = BufferResource.WithOffset(
                ...
                self.Workspace.RouteOutputBufferOffset(src_rank) + token_topk_idx * row_bytes)
            ...
            for col_half in l.static_range(2):
                ...
                BufferResource.StoreU32(output_row, col * 2, 0, value, BufferResource.kNTBit)
...
class SourceRouteReducer(DeviceTemplate):
    def __init__(self, Config, kGridBlocks=None, kBlockThreads=None):
        ...
        self.kTopK, self.kHiddenSize, self.kComputeHiddenSize = Config.kTopK, Config.kHiddenSize, Config.kComputeHiddenSize
        self.kElementsPerVec, self.kVecCols = 8, self.kHiddenSize // 8
        assert self.kThreads % 64 == 0 and self.kHiddenSize % 8 == 0 and self.kComputeHiddenSize >= self.kHiddenSize
    ...
    def Run(self, workspace, output, num_tokens, output_row_stride, sm_id, wid, wtid):
        ...
        if self.kNumRanks > 1:
            ...
            global_wave = l.where(num_tokens == 8, wid * self.kNumSMs + sm_id, sm_id * kWarpsPerBlock + wid)
        ...
        for wave_task in range(global_wave, total_wave_tasks, kTotalWaves):
            ...
            vec_in_wave = wtid
            while vec_in_wave < vecs_per_wave and wave_in_token * vecs_per_wave + vec_in_wave < self.kVecCols:
                ...
                for topk in l.static_range(self.kTopK):
                    row_offset = amdgcn_readfirstlane(route_row_offset + topk * self.kHiddenSize * 2)
                    route_values += (BufferResource.Load(workspace.br_, col_offset, row_offset, BufferResource.kSC1Bit | BufferResource.kNTBit),)
                ...
                for topk in l.static_range(self.kTopK):
                    for pair in l.static_range(kPackedElements):
                        ...
                        bf16 = ((word << 16).to(l.float32, bitcast=True), (word & 0xffff0000).to(l.float32, bitcast=True))
                        value = amdgcn_pk_add_f32(accum[pair], bf16)
                        accum = accum[:pair] + (value,) + accum[pair + 1:]
                packed_output = ()
                for pair in l.static_range(kPackedElements):
                    packed_output += (amdgcn_cvt_pk_bf16_f32(accum[pair][0], accum[pair][1]),)
                _store_vector4(output.to(l.pointer_type(l.uint32)) + (token * (output_row_stride // self.kElementsPerVec) + vec_col) * 4, packed_output)
                vec_in_wave += 64
