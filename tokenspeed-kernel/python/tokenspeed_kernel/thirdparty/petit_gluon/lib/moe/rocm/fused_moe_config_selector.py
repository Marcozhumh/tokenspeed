...
from tokenspeed_kernel.thirdparty.petit_gluon.lib.moe.rocm.memory_ops import MxFp4TileShape, TargetWeightLoadPolicy, BufferResource
from tokenspeed_kernel.thirdparty.petit_gluon.lib.moe.rocm.mem.input_mxfp4 import MxFp4Input
from tokenspeed_kernel.thirdparty.petit_gluon.lib.moe.rocm.mem.weight_mxfp4 import MxFp4Weights
from tokenspeed_kernel.thirdparty.petit_gluon.lib.moe.rocm.mem.bias import NoopBiasLayout, Bf16BiasLayout, MxFp4BiasLayout
...
class ConfigSelector(DeviceTemplate):
    def __init__(self, id, topk=4):
        ...
        self.kGroupN = 256
        self.kStage1GroupN = id.Stage1TileN() // 2 if id.stages == FusedMoEStages.kTwoStage else self.kGroupN
        ...
        self.kUseNonTemporalWeightLoads = (TargetWeightLoadPolicy.kAux == BufferResource.kNTBit
            if id.stages == FusedMoEStages.kOneStage else id.weight_load_policy == FusedMoEWeightLoadPolicy.kNonTemporal)
        self.kWeightLoadAux = BufferResource.kNTBit if self.kUseNonTemporalWeightLoads else BufferResource.kNone
        self.kActDType, self.kWeightDType, self.kMfmaShape = id.act_dtype, id.weight_dtype, id.mfma
        self.kValidateExpertIds = False
        self.kW13TileShape = {(32, 128): MxFp4TileShape.kN128, (32, 256): MxFp4TileShape.kN256,
                             (64, 256): MxFp4TileShape.kM64N256}[(self.kGroupM, self.kStage1GroupN)]
        ...
        self.ActivationOp = {FusedMoEActivationFunction.kSiluDot: SiluDotOp,
                             ...
                             FusedMoEActivationFunction.kKimiSitu: KimiSituOp}[id.activation]
        if id.act_dtype != FusedMoEDataType.kMxFp4 or id.weight_ordering != FusedMoEWeightOrdering.kNativeMxFp4:
        ...
        self.W13Weights, self.W2Weights = self.Weights.W13Weights, self.Weights.W2Weights
        ...
        if id.bias_dtype == FusedMoEDataType.kNone:
            ...
            self.Bias = Bf16BiasLayout(self.kNumWarps, self.kStage1GroupN,
                MxFp4BiasLayout(self.kGroupM, self.kStage1GroupN), self.kStage1GroupN // self.kStage1WarpsN // 16)
            self.Stage2Bias = Bf16BiasLayout(self.kNumWarps, self.kGroupN, MxFp4BiasLayout(32, self.kGroupN), 4)
        ...
        self.Stage1Op = (OnestageFusedMoEStage1DoubleBufferOp if id.stage1_buffering == FusedMoEStage1Buffering.kDoubleBuffer
