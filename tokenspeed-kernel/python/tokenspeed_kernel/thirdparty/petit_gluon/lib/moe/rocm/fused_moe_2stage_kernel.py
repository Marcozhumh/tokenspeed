...
from tokenspeed_kernel.thirdparty.petit_gluon.lib.tal.device import DeviceTemplate
from tokenspeed_kernel.thirdparty.petit_gluon.lib.tal.host_device import host_device
from tokenspeed_kernel.thirdparty.petit_gluon.lib.moe.rocm.ops.mxfp4_activation import MxFp4ActivationLayout
...
class TwoStageFusedMoEWorkspace(DeviceTemplate):
    ...
    def Bytes(self, route_groups, inter_dim):
        return self.ScaleOffset(route_groups, inter_dim) + self.ScaleRows(route_groups) * self.ScaleCols(inter_dim)


from typing import NamedTuple
import triton.experimental.gluon as g
from triton.experimental.gluon import language as l
from tokenspeed_kernel.thirdparty.petit_gluon.lib.tal.device import device_method
from tokenspeed_kernel.thirdparty.petit_gluon.lib.tal.thread_jit import thread_device_method
from tokenspeed_kernel.thirdparty.petit_gluon.lib.gemm.rocm.amd_intrinsics import _native_shared_memory
...
from tokenspeed_kernel.thirdparty.petit_gluon.lib.moe.rocm.ops.op_stages import TwoStageStage2Epilogue
from tokenspeed_kernel.thirdparty.petit_gluon.lib.gemm.rocm.amd_intrinsics import BufferResource, amdgcn_readfirstlane, amdgcn_s_setprio
...
class MxFp4Stage1WorkspaceEpilogue(DeviceTemplate):
    ...
    @device_method
    def Run(self, context, shm, h, tokens, row_metadata, route_base, route_group, expert_id, tile_n, tid, wid, wtid):
        ...
        for route_slice in l.static_range(self.Config.kGroupM // 8):
            ...
            self.StoreRoute(context, quant_shm, l.load(metadata + row), row, sorted_row, tile_n, col_lane)

    @device_method
    def StoreRoute(self, context, shm, fused, row, sorted_row, tile_n, col_lane):
        token, slot = fused & 0x00ffffff, fused >> 24
        valid = (sorted_row < context.num_valid_ids) & (token < context.m) & (slot < self.Config.kTopK)
        ...
        for col_segment in l.static_range(self.Config.kStage1GroupN // 128):
            ...
            self.Quantizer.Store(context.workspace, 0, context.scale_base, value_row,
                sorted_row, tile_n, quant_col_lane, self.Config.kInterDim,
                self.Workspace.ScaleCols(self.Config.kInterDim), quantized, valid=valid)


@g.jit
def TwoStageFusedMoEStage1Compute(workspace, act, w13, sorted_token_ids, sorted_expert_ids,
                                 num_valid_ids, scales_act, scales_w13, m, num_experts,
                                 max_num_m_blocks, w13_bias, Kernel: l.constexpr,
                                 Workspace: l.constexpr, kPersistent: l.constexpr):
    ...
    epilogue = Stage1EpilogueContext(MakeBufferResource(workspace, workspace_bytes), value_bytes,
                                    l.load(num_valid_ids), m)
    persistent_route_step = l.num_programs(1) if kPersistent else 0
    Kernel.Compute(act, w13, sorted_token_ids, sorted_expert_ids, scales_act, scales_w13,
                   num_valid_ids, m, num_experts, persistent_route_step, w13_bias, epilogue)
...
class TwoStageFusedMoEStage2(DeviceTemplate):
    def __init__(self, Config):
        ...
        self.kGroupDim, self.kNumWarps, self.kThreads = Config.kGroupDim, Config.kNumWarps, Config.kNumWarps * 64
        self.kTokenBatch = Config.kStage2TokenBatch
        self.kRoutesPerBlock = self.kTokenBatch * self.kNumWarps
        self.W2Weights, self.Bias, self.Stage2Tiles = Config.W2Weights, Config.Stage2Bias, Config.Stage2Tiles
    ...
    def Stage2(self, w2_weights, sorted_token_br, out, intermediate_ptr, route_group, max_route_groups,
               topk, num_tokens, sorted_weights, route_base, tile_n, tid, wid, wtid, expert_id, w2_bias, shm):
        ...
        packed_id = BufferResource.LoadU32(sorted_token_br, row * 4, 0, BufferResource.kNone)
        token, slot = packed_id & 0x00ffffff, packed_id >> 24
        valid = (token < num_tokens) & (slot < topk)
        value_voffset = l.where(valid, (token * topk + slot) * (self.kInterDim // 2) + vector * 16, 0)
        scale_voffset = route_group * self.kRoutesPerBlock * self.Stage2Input.kScaleCols
        output_bytes = num_tokens * self.Config.kDim * 2
        route_weights = MakeBufferResource(sorted_weights + route_base, self.kRoutesPerBlock * 4)
        if tid < self.kRoutesPerBlock:
            packed_token = BufferResource.LoadU32(sorted_token_br, 0, tid * 4, BufferResource.kNone)
            token, slot = packed_token & 0x00ffffff, packed_token >> 24
            output_row_offset = l.where((token < num_tokens) & (slot < topk), token * self.Config.kDim * 2, output_bytes)
            route_weight = BufferResource.LoadU32(route_weights, tid * 4, 0, BufferResource.kNone)
        ...
        for tile_k in l.static_range(self.kK256Tiles):
            stage: l.constexpr
            stage = tile_k & 1
            input_global = self.Stage2Input.LoadTile(workspace, value_voffset, 0, scale_voffset, values_bytes,
                                                     tile_k, valid, wtid)
            ...
            if tile_k + 1 == self.kK256Tiles:
                epilogue_route_weights = self.Stage2EpilogueOp.LoadRouteWeights(shm, wtid)
                epilogue_bias = self.Stage2EpilogueOp.PrefetchBias(tiles, tile_col, tid)
            else:
                l.barrier()
        l.barrier()
        amdgcn_s_setprio(0)
        accum = self.Stage2EpilogueOp.Apply(accum, epilogue_bias, epilogue_route_weights)
    ...
    @device_method
    def Compute(self, out, intermediate_ptr, w2, sorted_token_ids, sorted_weights, sorted_expert_ids,
                scales_w2, num_valid_ids_ptr, topk, num_experts, max_route_groups, w2_bias):
        tid = l.arange(0, self.kThreads, layout=l.BlockedLayout([1], [64], [self.kNumWarps], [0])).to(l.uint32)
        ...
        num_valid_ids, num_tokens = l.load(num_valid_ids_ptr), l.load(num_valid_ids_ptr + 1)
        route_group_limit = (num_valid_ids + self.kRoutesPerBlock - 1) // self.kRoutesPerBlock
        ...
        worker_route_groups = route_groups_per_worker + (persistent_worker < route_group_remainder).to(l.uint32)
        route_group_begin = persistent_worker * route_groups_per_worker + l.minimum(persistent_worker, route_group_remainder)
        for worker_tile in range(worker_route_groups):
            ...
            if (route_group < route_group_limit) & (route_base < num_valid_ids):
                expert_id = l.load(sorted_expert_ids + route_group // self.Config.kStage1ToStage2GroupRatio).to(l.uint32)
                valid_expert = (num_experts == 0) | (expert_id < num_experts)
                if valid_expert:
                    w2_weights = self.W2Weights.Initialize(w2, scales_w2, expert_id, tile_n, 0)
                    sorted_token_br = MakeBufferResource(sorted_token_ids + route_base, self.kRoutesPerBlock * 4)
                    self.Stage2(w2_weights, sorted_token_br, out, intermediate_ptr, route_group, max_route_groups,
                        topk, num_tokens, sorted_weights, route_base, tile_n, tid, wid, wtid, expert_id, w2_bias, shm)
            l.barrier()
...
@g.jit
def TwoStageFusedMoEStage2Compute(out, intermediate, w2, sorted_token_ids, sorted_weights,
                                 sorted_expert_ids, num_valid_ids, topk, scales_w2,
                                 num_experts, max_route_groups, w2_bias, Kernel: l.constexpr):
    Kernel.Compute(out, intermediate, w2, sorted_token_ids, sorted_weights, sorted_expert_ids,
                   scales_w2, num_valid_ids, topk, num_experts, max_route_groups, w2_bias)
