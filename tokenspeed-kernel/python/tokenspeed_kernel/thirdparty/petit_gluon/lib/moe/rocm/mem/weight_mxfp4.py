...
from tokenspeed_kernel.thirdparty.petit_gluon.lib.moe.rocm.memory_ops import MxFp4WeightLayout, MxFp4TileShape
...
class MxFp4W13(DeviceTemplate):
    def __init__(self, Config):
        ...
        for field in ('kScaleGroupK', 'kScaleGroupN', 'kWeightVecSize', 'kScaleBytesPerWord', 'kScaleWordBytes'):
    ...
    def Initialize(self, w13_base, scales_w13, expert_id, tile_k, dim, inter_dim):
        ...
        w1_ptr = (w13_base.to(l.pointer_type(l.uint8))
            + (expert_id * (2 * inter_dim * dim) // self.kWeightVecSize).to(l.uint64) * 16
            + (tile_k * self.W13.kGroupN * dim // self.kWeightVecSize).to(l.uint64) * 16)
        w13_scale_words_per_expert = (2 * dim * inter_dim) // kRowGroupSize // self.kScaleBytesPerWord
        scale_words_per_k_tile = (dim // self.kScaleGroupK) * self.kScaleGroupsPerOutputTile * 64
        scale_w1_ptr = scales_w13.to(l.pointer_type(l.uint32)) + expert_id * w13_scale_words_per_expert + tile_k * scale_words_per_k_tile
        ...
        w13_combined_scale_range = self.W13.ScaleProjectionOffsetBytes(dim, inter_dim) + w13_scale_range
        return W13State(self.W13.Initialize(w1_ptr, w13_combined_value_range, scale_w1_ptr, w13_combined_scale_range, dim))
...
class MxFp4W2(DeviceTemplate):
    ...
    def Initialize(self, w2, scales_w2, expert_id, tile_n, tile_k):
        ...
        scale_words_per_expert = kDim * kInterDim // kRowGroupSize // self.kScaleBytesPerWord
        value_tile_offset = tile_n * self.W2.kGroupN * kInterDim // 2 + tile_k * 2 * 64 * 16
        scale_tile_offset = tile_n * self.W2.kGroupN * kInterDim // kRowGroupSize // self.kScaleBytesPerWord + tile_k * 64
        value_ptr = w2.to(l.pointer_type(l.uint8)) + expert_id * value_bytes_per_expert + value_tile_offset
        scale_ptr = scales_w2.to(l.pointer_type(l.uint32)) + expert_id * scale_words_per_expert + scale_tile_offset
        return W2State(self.W2.Initialize(value_ptr, value_bytes_per_expert - value_tile_offset,
            scale_ptr, (scale_words_per_expert - scale_tile_offset) * self.kScaleWordBytes, kInterDim))
