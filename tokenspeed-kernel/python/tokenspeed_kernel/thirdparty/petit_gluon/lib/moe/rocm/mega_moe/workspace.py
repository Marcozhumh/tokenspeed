...
import tokenspeed_kernel.thirdparty.petit_gluon.lib.tal.host_device as _host_device_module
...
class MegaMoEWorkspace(DeviceTemplate):
    ...
    def __init__(self, Layout):
        ...
        self.kXGpuBarrierRecordBytes = 4 * 4096 if Layout.kNumExperts > 512 else (2 * 4096 if Layout.kNumExperts > 128 else 4096)
        ...
        self.kDirectWorkHeadBytes = self.kDirectWorkHeadSets * self.kDirectWorkShards * self.kDirectWorkShardStride
        ...
        self.kMaxPoolTokens = AlignUp( self.kNumRanks * self.kMaxTokensPerRank * self.kMaxExpertsPerToken + self.kNumExpertsPerRank * (self.kSortedTokenBlock - 1), self.kSortedTokenBlock)
        ...
        self.kDirectControlBytes = self.kDirectEntryCountBytes + self.kDirectPlanBaseBytes + self.kDirectCountDoneBytes + self.kDirectPlanReadyBytes + self.kDirectPayloadReadyBytes + self.kDirectEpochGateBytes + self.kDirectLaunchReadyBytes
        ...
        self.kRankSlotRawBytes = self.kNumRanks * self.kNumExpertsPerRank * 8 + self.kMaxTokensPerRank * Layout.kRouteOutputBufferBytes + self.kRouteOutputReadyBytes + self.kL1PayloadArrivalMaskBytes + self.kTokenMetadataBytes + self.kL1TokenBufferBytes + self.kL1TokenWeightBytes
        self.kSlotStride = AlignUp( self.kRankSlotRawBytes, self.kRankSymBufferBase)
        self.kLocalOffsetBase = AlignUp( self.kRankSymBufferBase + self.kNumRanks * self.kSlotStride, self.kLargePageBytes)
        self.kLocalDataBytes64 = self.kCacheLineBytes + self.kDirectWorkHeadBytes + self.kMaxTokensPerRank * self.kTopK * 4 + self.kNumExperts * 8 + self.kNumExpertsPerRank * 8 + self.kNumExperts * self.kMaxTokensPerRank * 4 + self.kMaxTokensPerRank * self.kTopK * 4 + self.kMaxTokensPerRank * Layout.kInputTokenBytes + self.kL2ArrivalMaskBytes + self.kL2TokenBufferBytes + self.kL2ScaleBufferBytes
        ...
        assert self.kDirectControlOffset + self.kDirectControlBytes <= self.kXGpuBarrierRecordBytes
    ...
    @property
    def cache_key(self):
        return sha256(Path(__file__).read_bytes() + Path(_host_device_module.__file__).read_bytes()).hexdigest() + "MegaMoEWorkspace(" + repr(sorted((k, v) for k, v in vars(self).items() if k != "Layout")) + ")"

    @device_method
    def Initialize(self, buf_base, rank_id):
        return WorkspaceState(MakeBufferResource(buf_base, self.kWorkspaceBytes), (l.full((), 0, l.uint32) + rank_id).to(l.uint32))
    ...
    def XGpuEpochSignalOffset(self, rank, source_rank):
        return rank * self.kXGpuBarrierRecordBytes + self.kXGpuEpochSignalBaseOffset + source_rank * 4

    @host_device
    def RecvCounterOffset(self, rank, src_rank, local_expert_idx):
        return self.RankOffsetBase(rank) + (src_rank * self.kNumExpertsPerRank + local_expert_idx) * 8

    @host_device
    def RouteOutputBufferOffset(self, rank):
        return self.RecvCounterOffset(rank, self.kNumRanks - 1, self.kNumExpertsPerRank - 1) + 8

    @host_device
    def RouteOutputReadyOffset(self, rank, parity, token):
        return self.RouteOutputBufferOffset(rank) + self.kMaxTokensPerRank * self.Layout.kRouteOutputBufferBytes + (parity * self.kMaxTokensPerRank + token) * 4

    @host_device
    def L1PayloadArrivalMaskOffset(self, rank, pool_block_index):
        return self.RouteOutputReadyOffset(rank, 1, self.kMaxTokensPerRank - 1) + 4 + pool_block_index * 4

    @host_device
    def TokenMetadataOffset(self, rank, pool_token_index):
        return self.L1PayloadArrivalMaskOffset(rank, self.kMaxPoolBlocks - 1) + 4 + pool_token_index * 8

    @host_device
    def L1TokenBufferOffset(self, rank, pool_token_index):
        return self.TokenMetadataOffset(rank, self.kMaxPoolTokens - 1) + 8 + pool_token_index * self.Layout.kInputTokenBytes

    @host_device
    def L1TokenWeightsOffset(self, rank, pool_token_index):
        return self.L1TokenBufferOffset(rank, self.kMaxPoolTokens - 1) + self.Layout.kInputTokenBytes + pool_token_index * 4

    @host_device
    def DirectPushEntryCountOffset(self, rank, block):
        return self.XGpuBarrierCounterOffset(rank) + self.kDirectControlOffset + block * 4

    @host_device
    def DirectPushPlanBaseOffset(self, rank, source_rank, local_expert_idx, parity=0):
        return self.DirectPushEntryCountOffset(rank, self.Layout.kNumSMs - 1) + 4 + (source_rank * self.kNumExpertsPerRank + local_expert_idx) * 8 + parity * 4

    @host_device
    def DirectPushCountDoneOffset(self, rank, parity, source_rank):
        return self.DirectPushPlanBaseOffset(rank, self.kNumRanks - 1, self.kNumExpertsPerRank - 1) + 8 + (parity * self.kNumRanks + source_rank) * 4

    @host_device
    def DirectPushPlanReadyOffset(self, rank, parity, destination_rank):
        return self.DirectPushCountDoneOffset(rank, 1, self.kNumRanks - 1) + 4 + (parity * self.kNumRanks + destination_rank) * 4

    @host_device
    def DirectPushPayloadReadyOffset(self, rank, parity, local_expert_idx):
        return self.DirectPushPlanReadyOffset(rank, 1, self.kNumRanks - 1) + 4 + (parity * self.kNumExpertsPerRank + local_expert_idx) * 4

    @host_device
    def DirectPushEpochGateOffset(self, rank):
        return self.DirectPushPayloadReadyOffset(rank, 1, self.kNumExpertsPerRank - 1) + 4
    ...
    def DirectPushWorkHeadOffset(self, shard, head_set = 0):
        return self.GridSyncBarrierOffset() + self.kCacheLineBytes + (head_set * self.kDirectWorkShards + shard) * self.kDirectWorkShardStride

    @host_device
    def InputTokenTopKExpertIDOffset(self):
        return self.GridSyncBarrierOffset() + self.kCacheLineBytes + self.kDirectWorkHeadBytes

    @host_device
    def SendCounterOffset(self, expert_idx):
        return self.InputTokenTopKExpertIDOffset() + self.kMaxTokensPerRank * self.kTopK * 4 + expert_idx * 8
    ...
    def RouteIndexOffset(self, destination, local_expert_idx, slot):
        return self.RecvSumCounterOffset(self.kNumExpertsPerRank - 1) + 8 + ((destination * self.kNumExpertsPerRank + local_expert_idx) * self.kMaxTokensPerRank + slot) * 4

    @host_device
    def InputTokenTopKExpertWeightOffset(self):
        return self.RouteIndexOffset(self.kNumRanks - 1, self.kNumExpertsPerRank - 1, self.kMaxTokensPerRank - 1) + 4

    @host_device
    def InputTokensOffset(self):
        return self.InputTokenTopKExpertWeightOffset() + self.kMaxTokensPerRank * self.kTopK * 4

    @host_device
    def L2ArrivalMaskOffset(self, pool_block_index):
        return self.InputTokensOffset() + self.kMaxTokensPerRank * self.Layout.kInputTokenBytes + pool_block_index * 4

    @host_device
    def L2TokenBufferOffset(self, pool_token_index):
        return self.L2ArrivalMaskOffset(self.kMaxPoolBlocks - 1) + 4 + pool_token_index * self.Layout.kInterDim // 2

    @host_device
    def L2ScaleBufferOffset(self):
        return self.L2TokenBufferOffset(self.kMaxPoolTokens - 1) + self.Layout.kInterDim // 2
